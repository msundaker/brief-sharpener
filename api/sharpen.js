...
1. why — is there a real reason THIS BRAND should be the one saying this, and does it close a genuine gap or shift in the market — or is it generic enough to fit any brand's campaign?
...
For each criterion give a verdict of exactly "sharp", "needs work", or "missing", plus one short, pointed question (max 15 words, one sentence) a senior strategist would ask aloud in the room to sharpen it. The question must interrogate that specific criterion's own definition above — never a generic question that could equally apply to a different criterion, and never one that strays into another criterion's territory (e.g. the why question is about brand relevance and market gap, not timing, and not the audience or the idea). Ground the question only in what the brief actually says — never presuppose a specific fact, date, or comparison (e.g. "versus last year") the brief didn't give you; when in doubt, ask the more general, foundational version of that criterion's question instead of inventing a specific angle. Skip the question only if verdict is "sharp" (use empty string).
